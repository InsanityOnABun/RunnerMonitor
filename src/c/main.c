...
static char s_step_buf[20];
static char s_battery_buf[30];
static char s_signal_buf[16];
...
static void canvas_update_proc(Layer *layer, GContext *ctx) {
  ...
  // bottom-right
  graphics_draw_line(ctx, GPoint(b.size.w - m, b.size.h - m),
                          GPoint(b.size.w - m - len, b.size.h - m));
  graphics_draw_line(ctx, GPoint(b.size.w - m, b.size.h - m),
                          GPoint(b.size.w - m, b.size.h - m - len));

  // Rule line above the time block - everything past here has stroke width of 1
  ...
  // Rule line below the time block - everything past here is always green
...
static void update_signal(void) {
  if (s_bt_connected) {
    ...
    snprintf(s_signal_buf, sizeof(s_signal_buf), "!! NO SIGNAL !!");
...
static void update_time(void) {
  ...
  strftime(s_time_buf, sizeof(s_time_buf),
           clock_is_24h_style() ? "%H:%M" : "%I:%M %p", t);
  text_layer_set_text(s_time_layer, s_time_buf);

  strftime(s_date_buf, sizeof(s_date_buf), "%a. %b %d %Y", t);
  snprintf(s_date_buf, sizeof(s_date_buf), "%s", s_date_buf);
...
static void window_load(Window *window) {
  ...
  text_layer_set_text(s_top_layer, "RUNNER.sys // MONITOR");
